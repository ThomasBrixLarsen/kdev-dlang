...
dlang::CodeCompletionModel *model = 0;
...
QList<CompletionTreeItemPointer> getCompletions(QString code)
{
	...
	dlang::CodeCompletionContext *completion = new dlang::CodeCompletionContext(DUContextPointer(context), code.mid(0, cursorIndex), cursor);
	model = new dlang::CodeCompletionModel(nullptr);
...
bool containsDeclaration(QString declaration, const QList<CompletionTreeItemPointer> &completions, int depth=-1, int quality=-1)
{
	...
	for(const CompletionTreeItemPointer item : completions)
	{
		...
		if(name.join(' ') == declaration && (depth == -1 || item->argumentHintDepth() == depth) && (quality == -1 || quality == itemQuality))
			return true;
	}
	//Print completions if we haven't found anything.
	debugPrint(completions, quality != -1);
	return false;
}
...
{
	QString code("modle main; void main(string[] args) { int a; %CURSOR }");
	auto completions = getCompletions(code);
	QCOMPARE(completions.size(), 3);
	QVERIFY(containsDeclaration(" main ()", completions)); //Function.
	QVERIFY(containsDeclaration("module main ", completions)); //Module.
