...
{
public:
	GoChanTypeData() : KDevelop::AbstractTypeData()
	{
		kind = 0;
	}
	GoChanTypeData(const GoChanTypeData &rhs) : KDevelop::AbstractTypeData(rhs),  valueType(rhs.valueType), kind(rhs.kind)
	{
...
class KDEVDDUCHAIN_EXPORT GoChanType: public KDevelop::AbstractType
{
public:
...
template<>
inline dlang::GoChanType *fastCast<dlang::GoChanType *>(AbstractType *from)
{
	if(!from || from->whichType() != AbstractType::TypeAbstract)
		return 0;
	return dynamic_cast<dlang::GoChanType *>(from);
}
