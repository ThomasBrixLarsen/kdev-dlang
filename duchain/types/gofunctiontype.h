...
class KDEVDDUCHAIN_EXPORT GoFunctionType : public KDevelop::FunctionType
{
public:
...
template<>
inline dlang::GoFunctionType *fastCast<dlang::GoFunctionType *>(AbstractType *from)
{
	if(!from || from->whichType() != AbstractType::TypeFunction)
		return 0;
	return dynamic_cast<dlang::GoFunctionType *>(from);
}
