...
void TestDuchain::test_declareVariables()
{
	...
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeInt));
	...
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeBool));
	decl = context->findDeclarations(QualifiedIdentifier("test3")).first();
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeRune));
	decl = context->findDeclarations(QualifiedIdentifier("test4")).first();
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeFloat64));
...
void TestDuchain::test_constants()
{
	...
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeFloat32));
	...
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeString));
	QVERIFY(decl->abstractType()->modifiers() & AbstractType::ConstModifier);
}
...
void TestDuchain::test_constants_omittedType()
{
	...
	DUChainReadLocker lock;
	Declaration *decl = context->findDeclarations(QualifiedIdentifier("const1")).first();
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeUint));
	QVERIFY(decl->abstractType()->modifiers() & AbstractType::ConstModifier);
	decl = context->findDeclarations(QualifiedIdentifier("const2")).first();
	QCOMPARE(fastCast<dlang::GoIntegralType *>(decl->abstractType().constData())->dataType(), uint(dlang::GoIntegralType::TypeUint));
	QVERIFY(decl->abstractType()->modifiers() & AbstractType::ConstModifier);
...
void TestDuchain::test_funcparams()
{
	...
	QVERIFY(decl);
	KDevelop::FunctionType::Ptr func = decl->abstractType().cast<KDevelop::FunctionType>();
	QVERIFY(func);
